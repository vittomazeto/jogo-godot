...
onready var exp_bar : TextureProgress = get_node("ExpBarBackgound/ExpBar")
...
func update_bar(type: String, value: int) -> void:
	match type:
		"HealthBar":
			...
			current_health = value #associar o novo valor a barra de vida
...
func call_tween(bar: TextureProgress, initial_value: int, final_value: int) -> void:
	var _interpolate_value: bool = tween.interpolate_property(
		...
		final_value,
		0.8,
		Tween.TRANS_QUAD, #interpola na potencia de 2, vai mais rapido no meio e lento na ponta
		Tween.EASE_IN_OUT #interpolação lenta no começo e no final
	)
