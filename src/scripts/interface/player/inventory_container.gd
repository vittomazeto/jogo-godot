...
func update_slot(item_name: String, item_image: StreamTexture, item_info: Array) -> void:
	var existing_item_index: int = slot_list.find(item_name) #procura se o item existe no inv.
	if existing_item_index != -1:
		...
		if item_slot.amount < 9 and item_slot.item_type != "Equipment" and item_slot.item_type != "Weapon":
			...
			if current_amount > 9:
				var leftover: int = current_amount - 9 # para ver se tem resto
				item_info[3] = 9 - item_slot.amount
				item_slot.update_item(item_name, item_image, item_info) # deixa 1 slot com 9
				item_info[3] = leftover
				update_slot(item_name, item_image, item_info) #repete a função ate preencher os slots
				return
			
			item_slot.update_item(item_name, item_image, item_info) #caso nao passe de 9
			return
	...
	for index in slot_container.get_child_count():
		...
		if  slot.item_name == "":
			...
			return #para nao adicionar o msm item em todos slots vazios
		
func empty_slot(index) -> void:
	slot_list[index] = ""
