...
func on_mouse_entered() -> void:
	can_click = true
	modulate.a = 0.5 #obter feedback que o objeto é clicavel
...
func update_item(item:String, item_image: StreamTexture, item_info: Array) -> void:
	...
	if amount != 0 and item_type != "Equipment" and item_type != "Weapon":
		item_amount.show() #mostrar item e quantidade
		item_texture.show()
		return
		
	if item_type == "Equipment" or item_type == "Weapon":
		item_texture.show() #mostra so o item
...
func _process(_delta: float) -> void:
	if Input.is_action_just_pressed("click") and can_click and item_name != "":
		...
		modulate.a = 0.5  #da efeito de click


func equip_item() -> void:
	if item_type == "Resource":
		return #nao vai executar mais nada abaixo
...
func update_slot() -> void:
	...
	emit_signal("empty_slot", item_index) #emite o sinal que o slot esvaziou pro inventory container	
